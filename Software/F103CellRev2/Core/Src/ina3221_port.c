...
static ina3221_status_t reg_read(uint8_t reg, uint16_t *out)
{
    ...
    /* Read two bytes */
    if (HAL_I2C_Master_Receive(&hi2c1, INA3221_ADDR_8BIT | 0x01u,
                                       ...
                                       ina3221_reading_t *out)
{
    ...
    /* --- Bus voltage ---
       Bits [15:3] are the value, bits [2:0] unused. LSB = 8mV. */
    st = reg_read(bus_reg, &raw);
    if (st != INA3221_OK) return st;
    out->voltage_mv = (int32_t)((int16_t)raw >> 3) * 8;

    /* --- Shunt voltage ---
       Bits [15:3] are signed two's complement, bits [2:0] unused. LSB = 40uV. */
    st = reg_read(shunt_reg, &raw);
    if (st != INA3221_OK) return st;
    out->shunt_uv = (int32_t)((int16_t)raw >> 3) * 40;

    /* --- Current (derived) ---
       I(mA) = Vshunt(uV) / Rshunt(mohm).
       Unit check: uV / mohm = uV / (mΩ) = mA. */
    uint32_t shunt_mohm;
    switch (ch)
    {
    ...
    out->current_ma = out->shunt_uv / (int32_t)shunt_mohm;
...
{
    /* Vshunt(uV) = I(mA) * R(mohm). Raw = Vshunt_uV / 40, shifted left 3. */
